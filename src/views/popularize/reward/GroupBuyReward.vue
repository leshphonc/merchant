...
export default {
  ...
  async mounted() {
    ...
    await this.readGroupBuyDetail(id).then(res => {
  ...
  methods: {
    ...
    ...mapActions('reward', ['updateGroupBuyReward']),
    ...mapActions('commodity', ['readGroupBuyDetail']),
    _controlTypePicker() {
    ...
    async _submit() {
      ...
      if (!isValid) {
        ...
        params.group_id = id
        this.updateGroupBuyReward(params)
          .then(() => {
            this.$toast.success({
              message: '操作成功',
              forbidClick: true,
