<template>
  <div>
    <van-nav-bar @click-left="$goBack" fixed left-arrow title="服务订单"></van-nav-bar>
      ...
      <van-list :finished="finished" :finished-text="finishText" @load="_onLoad" v-model="loading">
        <div :key="item.id" v-for="item in list">
          <van-panel
            ...
            :icon="item.goods_img"
            :status="_statusLabel(item.service_status, item.status)"
            :title="item.name"
          ...
          >
            ...
            <van-row>
              ...
              <van-col span="18">{{ item.price }} 元</van-col>
            </van-row>
            <div class="white-space"></div>
            <van-row>
...
export default {
  ...
  methods: {
    ...mapActions('order', ['getServiceList']),
    _changeOption(data) {
      this.find_type = data
      this._onRefresh()
    },
