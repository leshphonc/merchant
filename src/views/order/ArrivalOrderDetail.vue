<template>
  <div>
    ...
    <van-cell-group title="支付信息">
      ...
      <van-cell
        :value="`¥ ${detail.order.no_discount_money}`"
        title="不可优惠金额"
        v-if="detail.order.no_discount_money > 0"
      ></van-cell>
      <van-cell
        :value="`-¥ ${detail.order.card_price}`"
        title="商家优惠券"
        v-if="detail.order.card_price > 0"
      ></van-cell>
      <van-cell
        :value="`-¥ ${detail.order.coupon_price}`"
        title="系统优惠券"
        v-if="detail.order.coupon_price > 0"
      ></van-cell>
      <van-cell
        :value="`-¥ ${detail.order.score_deducte}`"
        title="积分抵扣"
        v-if="detail.order.score_deducte > 0"
      ></van-cell>
      <van-cell
        :value="`-¥ ${detail.order.card_give_money}`"
        title="会员卡赠送余额支付"
        v-if="detail.order.card_give_money > 0"
      ></van-cell>
      <van-cell
        :value="`-¥ ${detail.order.merchant_balance}`"
        title="商家余额支付"
        v-if="detail.order.merchant_balance > 0"
      ></van-cell>
      <van-cell
        :value="`-¥ ${detail.order.balance_pay}`"
        title="系统余额支付"
        v-if="detail.order.balance_pay > 0"
      ></van-cell>
      <van-cell
